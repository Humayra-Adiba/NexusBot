...
class Smile(commands.Cog):
    ...
    @nextcord.slash_command(name="smile", description="Share a lovely smile 😊")
    async def smile(self, interaction: Interaction):
        ...
        embed = nextcord.Embed(
            title=f"{interaction.user.display_name} is smiling freely!!!😊",
            color=nextcord.Color.green()
        )
        embed.set_image(url=random.choice(gifs))
        embed.set_footer(text="Your smile makes the world brighter!")
